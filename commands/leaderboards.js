...
var getHtmlTable = exports.getHtmlTable = function(room, n) {
	if (!isConfigured(room)) return null;
	var top = getTop(room);
	if (!top) return null;
	var htmlTable = "<table style='border: 1px solid #6688aa; border-radius: 10px'><tr><th style='border-bottom: 1px solid #94b8b8; padding: 5px'>#</th><th style='border-bottom: 1px solid #94b8b8; padding: 5px'>Name</th><th style='border-bottom: 1px solid #94b8b8; padding: 5px'>Score</th>";
	...
	htmlTable += "<th style='border-bottom: 1px solid #94b8b8; padding: 5px'>Tours Played</th></tr>";
	for (var i = 0; i < n && i < top.length; i++) {
		htmlTable += "<tr " + (i % 2 === 0 ? "" : "style='background-color: #cbcbcb; color: black'") + "><td>" + (i + 1) + "</td><td>" + top[i][0] + "</td><td align='right'>" + top[i][6] + "</td>";
		if (Settings.settings.leaderboards[room].winnerpoints !== 0) htmlTable += "<td align='right'>" + top[i][1] + "</td>";
		if (Settings.settings.leaderboards[room].finalistPoints !== 0) htmlTable += "<td align='right'>" + top[i][2] + "</td>";
		if (Settings.settings.leaderboards[room].semiFinalistPoints !== 0) htmlTable += "<td align='right'>" + top[i][3] + "</td>";
		if (Settings.settings.leaderboards[room].battlePoints !== 0) htmlTable += "<td align='right'>" + top[i][4] + "</td>";
		htmlTable += "<td align='right'>" + top[i][5] + "</td></tr>";
	}
	htmlTable += "</table>";
	return htmlTable
};
